...
const TreeGrid = () => {
  ...
  const fetchTrees = useCallback(async (p: number, s: SortOption) => {
    setLoading(true);
    try {
      const { data } = await api.get(`/products?${buildParams(p, s)}`);
      setTrees(data.products);
  ...
  useEffect(() => { fetchTrees(page, sort); }, [page, sort, fetchTrees]);

  // Auto-refetch when window regains focus (user comes back from admin panel)
  useEffect(() => {
    const handleFocus = () => fetchTrees(page, sort);
    window.addEventListener('focus', handleFocus);
    return () => window.removeEventListener('focus', handleFocus);
  }, [page, sort, fetchTrees]);
    ...
    <div className="w-full space-y-6">
      ...
      {loading || !trees ? (
        ...
        <div className="text-center py-16">
          <p className="text-gray-500 text-lg">No trees found.</p>
        </div>
      ) : (
        <>
          <motion.div
